...
- Поиск каталогов. Если ничего не найдено, появляется сообщение "Ничего не найдено".
...
- Создание каталога из загружаемого фида. При загрузке фида невалидного формата пользователь получает отчет об ошибке.
...
- Сортировка товаров. Сортировка по текстовым столбцам располагает данные в алфавитном порядке.
- Сортировка товаров. Сортировка по числовым столбцам располагает данные в порядке возрастания.
- Сортировка товаров. Сортировка по столбцам с датами располагает данные в хронологическом порядке.
- Изменение колонок таблицы товаров. При добавлении колонки в списке колонок она отображается в таблице товаров.
- Изменение колонок таблицы товаров. При удалении колонки из списка колонок она исчезает из таблицы товаров.
...
- Редактирование группы. После добавления товара в группу он отображается в списке товаров группы.
...
- Выбор регионов показа. После добавления вручную региона он отображается в списке регионов.
...
- Интересы и поведение. При вводе интересов появляется выпадающий список с интересами.
- Интересы и поведение. После добавления вручную региона он отображается в списке регионов.
...
<img src="screenshots/payment_options.PNG" width=300>

- При попытке пополнить счет с пустой суммой платежа появляется сообщение об ошибке.
